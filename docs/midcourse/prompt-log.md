...
| #                                    | Prompt                    | Output                                                          |
| ------------------------------------ | ------------------------- | --------------------------------------------------------------- |
| [1](#1--user-stories)                | User stories              | `docs/midcourse/user-stories.md`                                 |
| [2](#2--architecture-options)        | Two lightweight architectures | Option A / Option B proposal (input to the ADR)              |
| [3](#3--architecture-decision-record) | ADR                      | `docs/midcourse/mini-adr.md`                                     |
| [4](#4--backend--due-date-field)     | Due date field            | `app/models.py`, `app/storage.py`                                |
| [5](#5--backend--tags-field)         | Tags field                | `app/models.py`, `app/storage.py`                                |
| [6](#6--backend--update-path-rule)   | Update-path due-date rule | `app/business_rules.py`, `app/routers/tasks.py`                  |
| [7](#7--tests)                       | Pytest coverage           | `tests/test_due_dates.py`, `tests/test_tags.py`                  |
| [8](#8--frontend--modal-and-cards)   | Modal + card rendering    | `frontend/index.html`, `frontend/app.js`, `frontend/style.css`   |
| [9](#9--frontend--filter-bar)        | Overdue + tag filter bar  | `frontend/index.html`, `frontend/app.js`, `frontend/style.css`   |
...
```text
Write some user stories for a task tracker app. I want to add due dates and tags.
```
...
```text
What's a good architecture for adding due dates and tags to my FastAPI task tracker?
```
...
```text
Write an ADR for my task tracker. I picked the in-memory option over SQLite.
```
...
```text
Add a due_date field to my FastAPI task tracker's models and storage.
```
...
```text
Add tags to my FastAPI task tracker's models and storage.
```
...
```text
Make my task tracker reject a due date that's in the past when editing a task.
```
...
```text
Write pytest tests for the due date and tags features in my task tracker.
```
...
```text
Add due date and tags to the task modal and show them on the cards.
```
...
```text
Add a filter bar to my task tracker to filter by overdue tasks and by tag.
```
